...
{
  double captureMCsf = 0.005;
  double cosmicDatasf = 0.642;
  double inactiveLArMCsf = 0.15;
  TH1 *hpnsdataC4;
  TH1 *hpnsdataC3;
  TH1 *hpnsdataC2;
  TH1 *hpnsdataC1;
  ...
  myTree->Draw("XA5Det_CaponXA5top_drift5_20_yz_abs_30>>hsimsignalC4(50, 100, 2100)"); // Closest to PNS is ch5, correspond to C4 in CB
  hsimsignalC4 = (TH1*)gPad->GetPrimitive("hsimsignalC4");
  hsimsignalC4->Scale(captureMCsf);
  myTree->Draw("XA3Det_CaponXA3top_drift5_20_yz_abs_30>>hsimsignalC3(50, 100, 2100)");
  hsimsignalC3 = (TH1*)gPad->GetPrimitive("hsimsignalC3");
  hsimsignalC3->Scale(captureMCsf);
  myTree->Draw("XA2Det_CaponXA2top_drift5_20_yz_abs_30>>hsimsignalC2(50, 100, 2100)"); // Furthest to PNS is ch5, correspond to C2 in CB
  hsimsignalC2 = (TH1*)gPad->GetPrimitive("hsimsignalC2");
  hsimsignalC2->Scale(captureMCsf);
  myTree->Draw("XA4Det_CaponXA4top_drift5_20_yz_abs_30>>hsimsignalC1(50, 100, 2100)");
  hsimsignalC1 = (TH1*)gPad->GetPrimitive("hsimsignalC1");
  hsimsignalC1->Scale(captureMCsf);
  ...
  TH1F *hinactivebkgC4 = (TH1F*)c04->GetPrimitive("peothC4");
  hinactivebkgC4->Scale(inactiveLArMCsf);
  TCanvas *c03 = (TCanvas*)file3->Get("c03");
  TH1F *hinactivebkgC3 = (TH1F*)c03->GetPrimitive("peothC3");
  hinactivebkgC3->Scale(inactiveLArMCsf);
  TCanvas *c02 = (TCanvas*)file3->Get("c02");
  TH1F *hinactivebkgC2 = (TH1F*)c02->GetPrimitive("peothC2");
  hinactivebkgC2->Scale(inactiveLArMCsf);
  TCanvas *c01 = (TCanvas*)file3->Get("c01");
  TH1F *hinactivebkgC1 = (TH1F*)c01->GetPrimitive("peothC1");
  hinactivebkgC1->Scale(inactiveLArMCsf);
  // Fluka sim PNS signal
  //TH1F *hsimsignalC4 = (TH1F*)c04->GetPrimitive("pecap");
  //hsimsignalC4->Scale(0.13);
  ...
  legc4->AddEntry(hsimsignalC4, "capture signal (G4)", "f");
  ...
  legc3->AddEntry(hsimsignalC3, "capture signal (G4)", "f");
  ...
  legc2->AddEntry(hsimsignalC2, "capture signal (G4)", "f");
  ...
  legc1->AddEntry(hsimsignalC1, "capture signal (G4)", "f");
